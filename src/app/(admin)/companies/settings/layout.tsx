...
const navigation = [
	...
	{ name: "Billing", href: "/companies/settings/billing", current: false },
					...
					<div className="w-full shrink-0 lg:w-1/5">
						<div className="-mt-1 hidden flex-col pr-2 lg:flex">
							{navigation.map((item) => {
								...
								return (
									<Link key={item.name} href={item.href} className={classNames(isActive ? "text-brown-600 py-1 font-semibold" : "py-1 text-dark dark:text-white hover:text-gray-500 dark:hover:text-white/70")} aria-current={item.current ? "page" : undefined}>
										{item.name}
									</Link>
								);
							})}
