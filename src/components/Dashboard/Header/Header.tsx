...
export default function Nav({ navigation, userNavigation }: NavProps) {
	...
	return (
		<>
			<Disclosure as="nav" className="border-b border-gray-200 bg-white z-50">
				{({ open }) => (
					<>
						...
						<Disclosure.Panel className="sm:hidden">
							<div className="space-y-1 pt-2 pb-3">
								{navigation.map((item) => (
									<Disclosure.Button onClick={item.onClick ? () => signOut() : undefined} key={item.name} as="a" href={item.href} className={classNames(item.current ? "bg-indigo-50 border-indigo-500 text-indigo-700" : "border-transparent text-gray-600 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-800", "block pl-3 pr-4 py-2 border-l-4 text-base font-medium")} aria-current={item.current ? "page" : undefined}>
										{item.name}
									</Disclosure.Button>
								))}
							</div>
